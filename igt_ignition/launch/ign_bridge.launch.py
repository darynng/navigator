...
def generate_launch_description():
	...
	# infrared camera bridge 
	infrared_camera_bridge = Node(package='ros_gz_bridge', executable='parameter_bridge',
			namespace = namespace,
			name = 'infrared_camera_bridge',
			output='screen',
			parameters=[{
				'use_sim_time': use_sim_time
			}],
			arguments = [
				 ign_model_prefix + '/infrared_camera' + '@sensor_msgs/msg/Image' + '[ignition.msgs.Image'
			],
			remappings = [
				(ign_model_prefix + '/infrared_camera', '/infrared_camera')
			])
	...
	imu_bridge = Node(package='ros_gz_bridge', executable='parameter_bridge',
			...
			arguments = [
				 ign_model_prefix + '/imu' + '@sensor_msgs/msg/Imu' + '[ignition.msgs.IMU'
			],
			remappings = [
				(ign_model_prefix + '/imu', '/imu')
			])
	...
	odom_bridge = Node(package='ros_gz_bridge', executable='parameter_bridge',
			...
			arguments = [
				 ign_model_prefix + '/odom' + '@nav_msgs/msg/Odometry' + '[ignition.msgs.Odometry'
			])
